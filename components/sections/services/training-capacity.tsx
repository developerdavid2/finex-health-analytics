...
export default function TrainingCapacitySection() {
  ...
  max-md:after:hidden"
            >
              {trainingFeatures.map(
                ({ id, icon: IconComponent, caption, title, text, button }) => (
                  ...
                  >
                    ...
                    <p className="caption mb-5 max-md:mb-6 text-lg uppercase font-black text-main">
                      {caption}
                    </p>
                    <h2 className="max-w-400 mb-7 text-xl sm:text-3xl lg:text-5xl font-bold text-main text-p4 max-md:mb-6 max-md:h5">
                      {title}
                    </h2>
                    <p className="mb-11 max-md:mb-8 text-xl leading-relaxed text-main">
                      {text}
                    </p>
              ...
              <ul className="relative flex justify-around flex-grow px-[5%] border-2 border-gray-500/10 rounded-3xl max-md:hidden">
                ...
                {trainingDetails.map(({ id, icon: IconComponent, title }) => (
                  <li key={id} className="relative pt-16 px-4 pb-14">
                    ...
                    <div className="flex items-center justify-center mx-auto mb-3 border-2 border-gray-500/10 rounded-full hover:border-gray-500/40 transition-all duration-500 shadow-500 size-20 ">
                      <IconComponent className="text-3xl text-main" />
                    </div>
                    <h3 className="relative z-2 max-w-36 mx-auto my-8 text-base font-extrabold text-center uppercase text-main">
                      {title}
                    </h3>
                  </li>
                ))}
