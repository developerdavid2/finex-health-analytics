...
const ManagementCard: React.FC<ManagementCardProps> = ({
  ...
  return (
    <motion.div
      ...
      className="p-1.5 relative rounded-3xl border border-gray-400/20 overflow-hidden group opacity-70 hover:opacity-100 transition-all duration-300 bg-red-300/10"
    >
      {/* Text overlay */}
      ...
      <Image
        src={image}
        alt={name}
        className="w-[20rem] h-auto object-fill rounded-2xl transition-all duration-500 group-hover:scale-95 group-hover:blur-[10px]"
        width={300}
        height={300}
      />
    </motion.div>
